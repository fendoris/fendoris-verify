...
public final class FendorisVerify extends JavaPlugin {
    ...
    private void registerCommands() {
        ...
        if (verify != null) verify.setExecutor(new VerifyCommand(verifyService, messages, applicationService)); else getLogger().severe("Command 'verify' missing from plugin.yml");

        PluginCommand apply = getCommand("apply");
        if (apply != null) apply.setExecutor(new ApplyCommand(applicationService, messages)); else getLogger().severe("Command 'apply' missing from plugin.yml");
